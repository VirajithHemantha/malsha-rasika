...
export const Countdown: React.FC<CountdownProps> = ({ targetDate }) => {
  ...
  return (
    <section aria-label="Countdown" id="countdown" className="relative py-20 sm:py-28 px-6 overflow-hidden bg-[#080808]">
      ...
      <div className="mx-auto max-w-3xl text-center">
        <h2 className="font-serif text-3xl sm:text-5xl md:text-6xl mb-3" style={{ color: "rgb(232, 197, 71)", textShadow: "rgba(232, 197, 71, 0.3) 0px 0px 56px" }}>
          මංගල දිනයට තවත්
        </h2>
                ...
                <React.Fragment key={item.label}>
                  <div className="flex items-center gap-8 sm:gap-12 md:gap-14">
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-4xl sm:text-5xl md:text-6xl font-light leading-none" style={{ color: "rgb(232, 197, 71)", fontVariantNumeric: "tabular-nums", textShadow: "rgba(232, 197, 71, 0.28) 0px 0px 40px" }}>
                        {String(item.value).padStart(2, '0')}
                      </span>
                      <div className="w-full h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(212, 175, 55, 0.733), transparent)" }} />
                      <span className="text-[9px] sm:text-[10px] uppercase tracking-[0.38em] font-sans" style={{ color: "rgba(212, 175, 55, 0.6)" }}>
                        {item.label}
                      </span>
                    </div>
                    {index < arr.length - 1 && (
